...
export function SeverityCard({ severity, showLabel = true, className = '' }: SeverityCardProps) {
...
// ─── AI Severity Analyzer (visual component) ─────────────────
        ...
        <div className="mt-2 space-y-1">
          {['Scanning image...', 'Detecting issue type...', 'Calculating urgency...'].map((step, i) => (
            <div key={i} className="flex items-center gap-2">
              ...
              />
              <span className="text-xs text-teal-500 dark:text-teal-500">{step}</span>
            </div>
          ))}
        </div>
      </div>
